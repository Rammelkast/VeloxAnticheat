...
public final class AuraF extends MotionCheck {
	...
	/**
	 * This code is published under a different license Please read the copyright
	 * header above carefully
	 */
	...
	public void process(final Motion motion) {
		 ...
		 * 
		 * All values here are magic values obtained during testing with mainly
		 * LiquidBounce
		 */
		if (mod < 5e-4 && (factor > 0.0f && factor < 0.8f) && adjusted > 1e-3 && motion.getPitch() > 1.5f
			...
			verbose("factor: " + MathLib.roundDouble(factor, 2) + ", adjusted: " + MathLib.roundDouble(adjusted, 4)
					+ ", mod: " + MathLib.roundDouble(mod, 5) + ", pitch: "
					+ MathLib.roundDouble(motion.getPitch(), 1));
